...
import { resolverTicket } from "../../tickets/services/getTickets";

export const HeaderComent = ({ data }) => {
  const { id , estado} = data
  const [ticketEstado, setTicketEstado] = useState(estado)

  const handleResolver = async () => {
    try {
      const succes = await resolverTicket(id);

      if(succes){
        console.log("Ticket resuelto");
        setTicketEstado("resuelto");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="sticky top-0 z-10 bg-[#101322]/95 backdrop-blur-md border-b border-[#232948] px-8 py-5 flex flex-wrap justify-between items-center gap-4 shadow-lg shadow-black/20">
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-4 text-[#929bc9] text-sm mt-1">
          <span className="flex items-center gap-1">
            ...
            {data.dias_abiertos} Dias Abierto
          </span>
      ...
      <div className="flex items-center gap-3">
        <button className="flex items-center justify-center h-10 px-6 rounded-lg bg-green-500 hover:bg-green-500/90 text-white shadow-lg shadow-primary/20 transition-all text-sm font-bold gap-2 cursor-pointer" onClick={handleResolver}>
